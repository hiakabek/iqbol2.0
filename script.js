...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Chiziq tortish funksiyasi (Band stollar uchun)
  ...
  /* ==========================================================================
     CHECKOUT (OVQAT BUYURTMA QILISH)
     ========================================================================== */
  ...
  if (checkoutForm) {
    checkoutForm.addEventListener('submit', async (e) => {
      ...
      const BOT_TOKEN = '8634601019:AAEKyiMwJhM5py5e5Q7iiLQH0lezK3g66Ns'; const CHAT_ID = ['7225335915', '8072569639']; 
  ...
  if (reservationForm) {
    ...
    async function checkAndFilterBookedTables() {
      ...
      try {
        ...
        tableNumberSelect.innerHTML = '<option value="">Server yoniq emas</option>';
      }
    }
    ...
    reservationForm.addEventListener('submit', async (e) => {
      e.preventDefault();
      const name = document.getElementById('resName').value.trim(); const phone = document.getElementById('resPhone').value.trim(); const date = resDateInput.value; const time = resTimeSelect.value; const guests = document.getElementById('resGuests').value; const tableType = tableTypeSelect.value; const tableNumber = tableNumberSelect.value; const notes = document.getElementById('resNotes').value.trim();

      if (!name || !phone || !date || !time || !tableNumber) { showToast("Iltimos, barcha maydonlarni to'ldiring!", 'fa-exclamation-triangle'); return; }
      ...
      try {
        ...
        showToast("Xato! 'node server_2.js' orqali serverni yondiring.", 'fa-wifi');
      } finally {
        if (submitBtn) submitBtn.disabled = false;
      }
    });
  ...
  /* ==========================================================================
     10. SUCCESS MODAL (KO'K TUGMA YASHIRILDI)
     ========================================================================== */
  ...
  function showSuccessModal(data) {
    ...
    // Telegram ulash tugmasini butunlay yashirish
  ...
  /* ==========================================================================
     11. GALLERY, THEME & SCROLL
     ========================================================================== */
  const GALLERY_IMAGES = [
    { url: 'https://images.unsplash.com/photo-1544025162-d76694265947?q=80&w=1200&auto=format&fit=crop', caption: 'Tandir go\'shti', category: 'taomlar', featured: true },
    { url: 'https://images.unsplash.com/photo-1555939594-58d7cb561ad1?q=80&w=1200&auto=format&fit=crop', caption: 'Barra qo\'y shashliklari', category: 'shashlik', featured: false },
    { url: 'https://images.unsplash.com/photo-1582878826629-29b7ad1cdc43?q=80&w=1200&auto=format&fit=crop', caption: 'Qovurma uyg\'ur lag\'moni', category: 'taomlar', featured: false },
    { url: 'https://images.unsplash.com/photo-1592417817098-8f3d691029c0?q=80&w=1200&auto=format&fit=crop', caption: 'Sarxil salatlar', category: 'salatlar', featured: false }
  ];

  let currentGalleryCategory = 'all'; let activeLightboxIndex = 0; let activeGalleryItems = [];
  const galleryGrid = document.getElementById('galleryGrid'); const lightboxModal = document.getElementById('lightboxModal'); const lightboxImg = document.getElementById('lightboxImg'); const lightboxCaption = document.getElementById('lightboxCaption');
  ...
  function renderGallery() {
    ...
    galleryGrid.innerHTML = activeGalleryItems.map((item, index) => `<div class="gallery-item ${item.featured ? 'featured' : ''}" onclick="window.openLightbox(${index})"><img src="${item.url}" loading="lazy" /><div class="gallery-overlay"><span class="gallery-tag">${item.category}</span><h4 class="gallery-caption">${item.caption}</h4></div></div>`).join('');
  }

  document.querySelectorAll('.gallery-filter-btn').forEach(btn => { btn.addEventListener('click', () => { document.querySelectorAll('.gallery-filter-btn').forEach(b => b.classList.remove('active')); btn.classList.add('active'); currentGalleryCategory = btn.dataset.category; renderGallery(); }); });

  window.openLightbox = function(index) { activeLightboxIndex = index; const item = activeGalleryItems[activeLightboxIndex]; if (!item || !lightboxModal) return; lightboxImg.src = item.url; lightboxCaption.textContent = item.caption; lightboxModal.classList.add('open'); document.body.style.overflow = 'hidden'; };
  window.closeLightbox = function() { if (!lightboxModal) return; lightboxModal.classList.remove('open'); document.body.style.overflow = ''; };
  window.changeLightbox = function(step) { activeLightboxIndex += step; if (activeLightboxIndex < 0) activeLightboxIndex = activeGalleryItems.length - 1; if (activeLightboxIndex >= activeGalleryItems.length) activeLightboxIndex = 0; const item = activeGalleryItems[activeLightboxIndex]; if (item) { lightboxImg.src = item.url; lightboxCaption.textContent = item.caption; } };

  const savedTheme = localStorage.getItem('iqbol_theme') || 'light'; document.documentElement.setAttribute('data-theme', savedTheme);
  if (themeToggleBtn) { themeToggleBtn.innerHTML = savedTheme === 'dark' ? '<i class="fas fa-sun" style="color: #f3d889;"></i>' : '<i class="fas fa-moon"></i>'; themeToggleBtn.addEventListener('click', () => { const next = document.documentElement.getAttribute('data-theme') === 'dark' ? 'light' : 'dark'; document.documentElement.setAttribute('data-theme', next); localStorage.setItem('iqbol_theme', next); themeToggleBtn.innerHTML = next === 'dark' ? '<i class="fas fa-sun" style="color: #f3d889;"></i>' : '<i class="fas fa-moon"></i>'; }); }

  const header = document.getElementById('header'); window.addEventListener('scroll', () => { const scrollY = window.scrollY; if (header) header.classList.toggle('scrolled', scrollY > 50); if (backToTopBtn) backToTopBtn.classList.toggle('visible', scrollY > 400); });
  ...
  renderMenu(); updateCartUI(); renderGallery();
});